...
public class AsyncNotificationService {
    ...
    public void sendPurchaseNotification(String userEmail, String userName, String policyName, Double premium, Double coverage, LocalDate endDate) {
        log.info("📧 Processing purchase notification for: {}", userEmail);
        try {
            ...
            // Fallback to direct HTTP if RabbitMQ fails or throws exception
            fallbackHttp(userEmail, "SmartSure: Policy Purchase Successful", "Sync notification for policy update.");
        }
    }
    ...
    private void queueNotification(String email, String subject, String htmlBody) {
        try {
            ...
            log.error("Failed to queue to RabbitMQ: {}. Triggering fallback.", e.getMessage());
            fallbackHttp(email, subject, htmlBody);
        }
    }

    private void fallbackHttp(String email, String subject, String htmlBody) {
        try {
            log.info("🚀 Triggering HTTP fallback for: {}", email);
            notificationClient.sendEmail(new EmailRequest(email, subject, htmlBody));
        } catch (Exception ex) {
            log.error("❌ Fatal: HTTP Fallback also failed: {}", ex.getMessage());
        }
    }
}
